...
**jdk**：jvm + Java API类库 + 编译（Javac）、执行（java）、调试（jstack、jmap、jhat、jps、jstat）、打包（jar）等一系列工具链
...
- **jmap（Memory map for java）**：生成虚拟机的内存转储快照（heapdump文件），如果不使用jmap要想获取java堆转储快照，可以使用-XX：+HeapDumpOnOutOfMemoeryError，让虚拟机在OOM异常的时候自动生成dump文件，或通过-XX:+HeapDumpOnCtrlBreak参数可以使用【ctrl】+[break]快捷键让虚拟机生成dump文件；又或者在**Linux系统下通过kill -3 命令**发送进程退出信号『吓唬』一下虚拟机，也能拿到dump文件
