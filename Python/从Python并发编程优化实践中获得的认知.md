...
从上面的并发编程实践我们可以看到，Python提供了线程、进程和协程的并发编程方式，但不同的并发实践需要应用到不同的场景才能起到效果，我们的系统有两种典型的场景：CPU密集型和IO密集型，其实我们的web应用系统基本上都是IO密集型，因此多线程和协程是可以起到不错的优化效果的，但是Python的协程会受到异步库支持的限制，大家使用的时候需要注意一下。
